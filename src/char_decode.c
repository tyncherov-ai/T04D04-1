...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    fprintf(stderr, "Ошибка: необходимо указать режим (0 - кодирование, 1 - "
                    "декодирование).\n");
    return 1;
  }
  ...
  if (argv[1][0] == '0' && argv[1][1] == '\0') {
    ...
    fprintf(
        stderr,
        "Ошибка: режим должен быть 0 (кодирование) или 1 (декодирование).\n");
    return 1;
  }
  ...
  if (fgets(buffer, sizeof(buffer), stdin) != NULL) {
    ...
    if (!has_space && mode == 0) {
      printf(
          "Ошибка: необходимо вводить буквы через пробел для кодирования.\n");
      return 1;
    }
